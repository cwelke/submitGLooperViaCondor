...
filesList=`ls $CMS2NtupleLocation`
...
for list in $filesList; do
	...
	fileIn=${CMS2NtupleLocation}$list
	inputArguments="$CMSSWRelease $fileIn $outputDir $CMS2Tag $CMS2Tar $jobnumber $dataset $isdata"
	inputFiles=cms2tar/$CMS2Tar,looperFiles/libmakePhotonBabies.so,looperFiles/runMakePhotonBabies.cc,looperFiles/Merged_190456-208686_8TeV_PromptReReco_Collisions12_goodruns.txt,looperFiles/vtxreweight_Summer12MC_PUS10_19fb_Zselection.root,$executableScript
# looperFiles/makePhotonBabies.h,looperFiles/makePhotonBabies.cc,looperFiles/ZHPhotonLooper.tgz
	outFileName=unmerged_ntuple_${jobnumber}.root
...
# outputfilename="ntuple_jobnumber"
	...
	ls -l $outputDir/unmerged/$outFileName > /dev/null 2>&1
	doesExist=$?
	if [ $doesExist == 0 ]; then
